...
const submit = async () => {
  ...
  try {
    ...
    if (state.dialogTitle === "Edit") {
      method = 'put'
      url = '/perusahaan/' + state.formItem.id
    }
    ...
    let perusahaanDatas = store.state.perusahaan
    if (state.dialogTitle === "Edit") {
      let findIndex = store.state.perusahaan.findIndex(dataItem => dataItem.id == state.formItem.id)
      perusahaanDatas[findIndex] = response.data
    } else {
      perusahaanDatas.push(response.data) 
    }
    ...
    store.commit('setData', {
      field: 'perusahaan',
      value: perusahaanDatas
    })

    store.commit('setData', {
      field: 'perusahaanRef',
      value: store.state.perusahaan.map(itemRef => itemRef.nama)
    })
...
const confirmDelete = (rowData) => {
  $q.dialog({
    ...
    try {
      ...
      let response = await axios.delete('/perusahaan/' + rowData.id)
      response = await response.data
      
      if (!response.data) throw response.message
    
      let newData = store.state.perusahaan
      let dataIndex = store.state.perusahaan.findIndex(dataItem => dataItem.id === rowData.id)
...
const getData = async () => {
  state.loading = true
  try {
    let response = await axios.get('/perusahaan')
    response = await response.data
...
<template>
  ...
  >
    <template v-slot:top>
      <div class="text-h5">Data Perusahaan</div>
      <q-space />
    ...
    <q-card style="width: 700px; max-width: 80vw;">
      ...
      <q-card-section class="q-pt-none">
        <q-form @submit.prevent="submit">
          <q-input outlined v-model="state.formItem.nama" autofocus type="text" label="Nama Perusahaan" color="primary">
          </q-input>

          <q-input outlined v-model="state.formItem.kode" autofocus type="text" label="Kode Perusahaan" color="primary">
          </q-input>
